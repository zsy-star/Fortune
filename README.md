...
清空订单、批量删除订单、清空日志、重置开奖记录第一阶段已开放，但必须受保护执行，不是普通日常功能，也不是完整角色权限系统。统一要求：操作前自动备份，二次确认，原因必填，确认短语匹配，成功或失败都要写操作日志；清空日志需先归档日志，重置开奖记录在存在结算记录时阻断。

小范围试用时不建议频繁使用这些入口；确需使用时，应先手工确认备份，再按界面保护流程操作。当前版本仍依赖本机管理和操作规范，权限系统 / 登录 / 角色 / 审批尚未作为完整功能交付。

## 仍未开放或未冻结能力

- 权限系统：角色、授权范围和管理员审计尚未作为完整系统交付。
